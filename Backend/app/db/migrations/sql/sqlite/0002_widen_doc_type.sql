-- Widen documents.document_type CHECK to include 'bank_pdf' and 'unknown_pdf'.
--
-- Per FILING_FLOW.md §3.3, the enum includes `bank_pdf`. The initial migration
-- shipped without it (and without `unknown_pdf`, which the type-detector emits
-- for PDFs whose content sniff can't classify them confidently before Gemini
-- runs in Step 3).
--
...
CREATE TABLE documents_new (
    ...
    CONSTRAINT chk_doc_type CHECK (document_type IN (
        'form16','bank_csv','bank_pdf','ais_tis','form_26as','salary_slip','unknown_pdf'
    )),
...
SELECT id, user_id, filing_id, tax_year,
       ...
       extraction_started_at, extraction_finished_at, extraction_error,
